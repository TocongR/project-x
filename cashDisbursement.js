let api = "https://script.google.com/macros/s/AKfycbwqlRrZasNWJsTX52DZnPRLboFUPll8Bzd3KFuXlv5ljVBjJl5Cl-flWGKPdutIpsjI/exec";
...
function readData() {
    ...
    .then(res => res.json())
    .then(dataCd => {
        let cdData = dataCd.cdData;
        let trtd = cdData.reverse().map(each => {
...
function delData(cashDisbursementId) {
    const deleteBtn = document.querySelector(".delete");
    deleteBtn.textContent = "Deleting...";

    const url = api + `?del=true&cashDisbursementId=${cashDisbursementId}`;
...
function updateData(cashDisbursementId) {
    if (!validateForm()) return;

    const updateBtn = document.querySelector(".update");
    updateBtn.textContent = "Updating..";

    fetch(api + `?update=true&cashDisbursementId=${cashDisbursementId}&customer=${form[0].value}&reference=${form[1].value}&datePaid=${form[2].value}&sourceDoc=${form[3].value}&discount=${form[4].value}&particulars=${form[5].value}&amount=${form[6].value}&wTax=${form[7].value}`)
    .then(res => res.text())
    .then(() => {
        readData();
        form.reset();
